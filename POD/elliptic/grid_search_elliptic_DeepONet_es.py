...
if __name__ == "__main__":
    ...
    for N_drop_ in [100, 200]:
        for N_basis_ in [100, 200]:
            for N_layers_branch_ in [4, 3]:
                for N_encoder_trunk_ in [5, 4]:
                    for learning_rate_ in [1e-3, 1e-4]:
                        for kernel_size_trunk_ in [7, 3]:
                            args["N_drop"] = N_drop_
                            args["N_basis"] = N_basis_
                            args["N_layers_branch"] = N_layers_branch_
                            args["learning_rate"] = learning_rate_
                            args["kernel_size_trunk"] = kernel_size_trunk_
                            args["N_encoder_trunk"] = N_encoder_trunk_
                            ...
                            for nn_ in nn:
                                ...
                                _, predictions = scan(make_prediction_scan, [model, features[args["N_train"]:(args["N_train"]+args["N_val"])], coordinates, coordinates], jnp.arange(features[args["N_train"]:(args["N_train"]+args["N_val"])].shape[0]))
                            ...
                            _, predictions = scan(make_prediction_scan, [models[best_n], features, coordinates, coordinates], jnp.arange(features.shape[0]))
