...
if __name__ == "__main__":
    ...
    a_features = data['features']

    data = np.load(args['dataset_reg_path'])
    targets = data['solutions']
    rhs = data['rhs']
    
    N_samples = rhs.shape[0]
    features = jnp.concatenate([rhs.reshape(N_samples, 1, -1), a_features.reshape(N_samples, a_features.shape[1], -1)], axis=1)
    features = features / jnp.max(jnp.abs(features), axis=[0, 2], keepdims=True)
    ...
    for N_basis_f_ in [100, 200, 300, 400, 500]:
        ...
        for N_basis_t_ in [100, 200, 300, 400]:
