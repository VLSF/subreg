...
class FFNO_OAL(eqx.Module):
    ...
    def __call__(self, u, x, eps=1e-4):
        ...
        u = self.decoder(u)
        D = jnp.sum(jnp.abs(u.reshape(u.shape[0], -1) @ u.reshape(u.shape[0], -1).T), axis=1)
        D = jnp.sqrt((D + (D <= eps)))
        u = u / D.reshape([-1, ]+ [1,]*(u.ndim-1))
        return u
...
def stochastic_normal_loss_averaged(model, input, target, x, eps=1e-4):
    target = target.reshape(target.shape[0], -1).T
    basis = model(input, x)
    basis = basis.reshape(basis.shape[0], -1)
    b = basis @ target
    A = basis @ basis.T
    sol = basis.T @ jnp.linalg.solve(A + jnp.eye(A.shape[0])*eps, b)
    return jnp.mean(jnp.linalg.norm(target - sol, axis=0)**2)

def batch_stochastic_normal_loss_averaged(model, input, target, x):
    res = vmap(stochastic_normal_loss_averaged, in_axes=(None, 0, 0, None))(model, input, target, x)
    return jnp.mean(res)

stochastic_normal_averaged_compute_loss_and_grads = eqx.filter_value_and_grad(batch_stochastic_normal_loss_averaged)

def stochastic_normal_averaged_make_step_scan(carry, n, optim, n_average):
    model, features, targets, x, key, opt_state = carry
    key, key_ = random.split(key)
    coeff = random.normal(key_, (n.shape[0], n_average, targets.shape[1], 1))
    targets_ = jnp.sum(jnp.expand_dims(targets[n], axis=1)*coeff, axis=2)
    loss, grads = stochastic_normal_averaged_compute_loss_and_grads(model, features[n], targets_, x)
    grads = tree_map(lambda x: x.conj(), grads)
    updates, opt_state = optim.update(grads, opt_state, eqx.filter(model, eqx.is_array))
    model = eqx.apply_updates(model, updates)
    return [model, features, targets, x, key, opt_state], loss
...
def subspace_loss_normal(model, input, target, x, eps=1e-4):
    Q = model(input, x)
    Q = Q.reshape(Q.shape[0], -1)
    G = Q @ Q.T
    G = jnp.linalg.inv(G + jnp.eye(G.shape[0])*eps)
    Q = target.reshape(target.shape[0], -1) @ Q.T
    return target.shape[0] - jnp.trace(Q @ G @ Q.T)

def batch_subspace_loss_normal(model, input, target, x):
    res = vmap(subspace_loss_normal, in_axes=(None, 0, 0, None))(model, input, target, x)
    return jnp.mean(res)

subspace_normal_compute_loss_and_grads = eqx.filter_value_and_grad(batch_subspace_loss_normal)

def subspace_normal_make_step_scan(carry, n, optim):
    model, features, targets, x, opt_state = carry
    loss, grads = subspace_normal_compute_loss_and_grads(model, features[n], targets[n], x)
    grads = tree_map(lambda x: x.conj(), grads)
    updates, opt_state = optim.update(grads, opt_state, eqx.filter(model, eqx.is_array))
    model = eqx.apply_updates(model, updates)
    return [model, features, targets, x, opt_state], loss

def subspace_loss_fro(model, input, target, x, eps=1e-4):
    Q = model(input, x)
    Q = Q.reshape(Q.shape[0], -1)
    Q_ = target.reshape(target.shape[0], -1) @ Q.T
    return 1 - jnp.trace(Q_ @ Q_.T) / jnp.trace(Q @ Q.T)

def batch_subspace_loss_fro(model, input, target, x):
    res = vmap(subspace_loss_fro, in_axes=(None, 0, 0, None))(model, input, target, x)
    return jnp.mean(res)

subspace_fro_compute_loss_and_grads = eqx.filter_value_and_grad(batch_subspace_loss_fro)

def subspace_fro_make_step_scan(carry, n, optim):
    model, features, targets, x, opt_state = carry
    loss, grads = subspace_fro_compute_loss_and_grads(model, features[n], targets[n], x)
    grads = tree_map(lambda x: x.conj(), grads)
    updates, opt_state = optim.update(grads, opt_state, eqx.filter(model, eqx.is_array))
    model = eqx.apply_updates(model, updates)
    return [model, features, targets, x, opt_state], loss
...
def OAL_loss(model, input, target, x):
    Q = model(input, x)
    Q = Q.reshape(Q.shape[0], -1).T
    return target.shape[0] - jnp.sum((target.reshape(target.shape[0], -1) @ Q)**2)

def batch_OAL_loss(model, input, target, x):
    res = vmap(OAL_loss, in_axes=(None, 0, 0, None))(model, input, target, x)
    return jnp.mean(res)

OAL_compute_loss_and_grads = eqx.filter_value_and_grad(batch_OAL_loss)

def OAL_make_step_scan(carry, n, optim):
    model, features, targets, x, opt_state = carry
    loss, grads = OAL_compute_loss_and_grads(model, features[n], targets[n], x)
    grads = tree_map(lambda x: x.conj(), grads)
    updates, opt_state = optim.update(grads, opt_state, eqx.filter(model, eqx.is_array))
    model = eqx.apply_updates(model, updates)
    return [model, features, targets, x, opt_state], loss
    ...
    args = {
       "-dataset_name": {
        ...
        "-experiment_type": {
            "default": "l2",
            "type": str,
            "choices": ["l2", "stochastic_normal", "subspace", "subspace_normal", "subspace_fro", "stochastic_normal_averaged", "OAL"],
        ...
        "-N_average": {
            "default": 1,
            "type": int,
            "help": "number of samples used to compute average for 'stochastic_normal_averaged' loss function"
        },
        "-N_subspace": {
            "default": 10,
            "type": int,
...
if __name__ == "__main__":
    ...
    if not os.path.isfile(f'grid_search_Burgers_OAL/{args["experiment_type"]}/results.csv'):
        with open(f'grid_search_Burgers_OAL/{args["experiment_type"]}/results.csv', "w") as f:
            f.write(header)

    data = jnp.load(f'Burgers_{args["dataset_name"]}.npz')
    subspace_data = jnp.load(f'Burgers_{args["dataset_name"]}_local_POD.npz')
            ...
            for N_layers_ in [5, 4, 3]:
                for learning_rate_ in [1e-3, 1e-4]:
                    ...
                    if args["experiment_type"] == "l2":
                        ...
                        model = FFNO_OAL(args["N_layers"], N_features, args["N_modes"], D, keys[0])
                    else:
                    ...
                    if args["experiment_type"] == "stochastic_normal" or args["experiment_type"] == "stochastic_normal_averaged":
                    ...
                    elif args["experiment_type"] == "OAL":
                        make_step_scan_ = lambda a, b: OAL_make_step_scan(a, b, optim)
                    elif args["experiment_type"] == "subspace_normal":
                        make_step_scan_ = lambda a, b: subspace_normal_make_step_scan(a, b, optim)
                    elif args["experiment_type"] == "subspace_fro":
                        make_step_scan_ = lambda a, b: subspace_fro_make_step_scan(a, b, optim)
                    elif args["experiment_type"] == "stochastic_normal_averaged":
                        make_step_scan_ = lambda a, b: stochastic_normal_averaged_make_step_scan(a, b, optim, args["N_average"])
                    ...
                    eqx.tree_serialise_leaves(f'grid_search_Burgers_OAL/{args["experiment_type"]}/model_{exp_hash}.eqx', model)
                    eqx.tree_serialise_leaves(f'grid_search_Burgers_OAL/{args["experiment_type"]}/opt_state_{exp_hash}.eqx', opt_state)
                
                    cosines, errors, rel_errors = compute_metrics(model, features, u0, forcing, targets, coordinates, basis, a_extended)
                    ...
                    with open(f'grid_search_Burgers_OAL/{args["experiment_type"]}/results.csv', "a") as f:
                        f.write(data)
                    
                    jnp.savez(f'grid_search_Burgers_OAL/{args["experiment_type"]}/metrics_{exp_hash}.npz', cosines=cosines, errors=errors, eigvecs_errors=rel_errors, history=history)