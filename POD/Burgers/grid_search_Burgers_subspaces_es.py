...
if __name__ == "__main__":
    ...
    if not os.path.isfile(f'grid_search_Burgers_subspaces_es/{args["experiment_type"]}/results.csv'):
        with open(f'grid_search_Burgers_subspaces_es/{args["experiment_type"]}/results.csv', "w") as f:
            f.write(header)
        ...
        for N_modes_ in [16, 14, 10]:
            for N_layers_ in [5, 4, 3]:
                for learning_rate_ in [1e-3, 1e-4]:
                    ...
                    eqx.tree_serialise_leaves(f'grid_search_Burgers_subspaces_es/{args["experiment_type"]}/model_{exp_hash}.eqx', models[best_n])
                    eqx.tree_serialise_leaves(f'grid_search_Burgers_subspaces_es/{args["experiment_type"]}/opt_state_{exp_hash}.eqx', opt_states[best_n])
                    ...
                    with open(f'grid_search_Burgers_subspaces_es/{args["experiment_type"]}/results.csv', "a") as f:
                        f.write(data)
                    
                    jnp.savez(f'grid_search_Burgers_subspaces_es/{args["experiment_type"]}/metrics_{exp_hash}.npz', cosines=cosines, errors=errors, eigvecs_errors=rel_errors, history=history)