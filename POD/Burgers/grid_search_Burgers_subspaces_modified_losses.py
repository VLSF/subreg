...
def subspace_loss_normal(model, input, target, x, eps=1e-4):
    Q = model(input, x)
    G = Q.reshape(Q.shape[0], -1) @ Q.reshape(Q.shape[0], -1).T
    ...
    args = {
       "-dataset_name": {
        ...
        "-experiment_type": {
            "default": "l2",
            "type": str,
            "choices": ["l2", "stochastic_normal", "subspace", "subspace_normal", "stochastic_normal_averaged"],
...
if __name__ == "__main__":
    ...
    if not os.path.isfile(f'grid_search_Burgers_modified_losses/{args["experiment_type"]}/results.csv'):
        with open(f'grid_search_Burgers_modified_losses/{args["experiment_type"]}/results.csv', "w") as f:
            f.write(header)
        ...
        for N_modes_ in [16, 14, 10]:
            for N_layers_ in [5, 4, 3]:
                for learning_rate_ in [1e-3, 1e-4]:
                    ...
                    eqx.tree_serialise_leaves(f'grid_search_Burgers_modified_losses/{args["experiment_type"]}/model_{exp_hash}.eqx', model)
                    eqx.tree_serialise_leaves(f'grid_search_Burgers_modified_losses/{args["experiment_type"]}/opt_state_{exp_hash}.eqx', opt_state)
                    ...
                    with open(f'grid_search_Burgers_modified_losses/{args["experiment_type"]}/results.csv', "a") as f:
                        f.write(data)
                    
                    jnp.savez(f'grid_search_Burgers_modified_losses/{args["experiment_type"]}/metrics_{exp_hash}.npz', cosines=cosines, errors=errors, eigvecs_errors=rel_errors, history=history)