...
if __name__ == "__main__":
    ...
    header += ",hash,final_loss,model_size,training_time,train_eigvecs_error,test_eigvecs_error"

    if not os.path.isfile(f'grid_search_Burgers_DeepPOD/{args["experiment_type"]}/results.csv'):
        with open(f'grid_search_Burgers_DeepPOD/{args["experiment_type"]}/results.csv', "w") as f:
            f.write(header)
            ...
            for N_layers_ in [5, 4, 3]:
                for learning_rate_ in [1e-3, 1e-4]:
                    ...
                    if args["experiment_type"] == "stochastic_normal":
                        carry = [model, features, targets_, coordinates, keys[1], opt_state]
                    else:
                    ...
                    eqx.tree_serialise_leaves(f'grid_search_Burgers_DeepPOD/{args["experiment_type"]}/model_{exp_hash}.eqx', model)
                    eqx.tree_serialise_leaves(f'grid_search_Burgers_DeepPOD/{args["experiment_type"]}/opt_state_{exp_hash}.eqx', opt_state)
                
                    rel_errors = compute_metrics(model, features, u0, forcing, targets, coordinates, a_extended)
                    ...
                    with open(f'grid_search_Burgers_DeepPOD/{args["experiment_type"]}/results.csv', "a") as f:
                        f.write(data)
                    
                    jnp.savez(f'grid_search_Burgers_DeepPOD/{args["experiment_type"]}/metrics_{exp_hash}.npz', eigvecs_errors=rel_errors, history=history)