...
def get_dataset(N, N_samples, N_eigenvalues, params, key, N_coarse, maxiter):
    ...
    for keys in Keys:
        A, a1, data = get_matrix(discretization_data, params, keys[0])
        ...
        features.append(np.expand_dims(a1, 0))
        Data.append(data)
...
if __name__ == "__main__":
    ...
    for i, key in enumerate(keys):
        data = get_dataset(N, N_samples_, N_eigenvalues, params, key, N_coarse, maxiter)
        jnp.savez(f"lone_flattop_low_res/lone_flattop_{i}.npz", eigenvalues=data["eigenvalues"], eigenvectors=data["eigenvectors"], features=data["features"], A_data=data["A_data"])
        if i == 0:
            jnp.savez(f"lone_flattop_low_res/lone_flattop_global.npz", A_indices=data["A_indices"], coordinates=data["coordinates"])