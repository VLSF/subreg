...
def get_params(experiment_type):
    if experiment_type == "subspace":
        params = {
            10: {
                "learning_rate": [1e-4, 1e-3, 1e-3],
                "N_drop": [100, 100, 200],
                "N_modes": [14, 14, 10],
                "N_layers": [4, 4, 5]
            },
    ...
    elif experiment_type == "stochastic_normal":
        params = {
            10: {
                "learning_rate": [1e-3, 1e-3, 1e-3],
                "N_drop": [200, 100, 200],
...
if __name__ == "__main__":
    ...
    params_ = get_params(args["experiment_type"])

    for N_subspace_ in params_.keys():
        args["N_subspace"] = N_subspace_
        for ii in range(3):
            for key_ in params_[N_subspace_].keys():
                args[key_] = params_[N_subspace_][key_][ii]
            ...
            if not os.path.isfile(f'optimal/{args["experiment_type"]}/results.csv'):
                with open(f'optimal/{args["experiment_type"]}/results.csv', "w") as f:
                    f.write(header)
            ...
            eqx.tree_serialise_leaves(f'optimal/{args["experiment_type"]}/model_{exp_hash}.eqx', model)
            eqx.tree_serialise_leaves(f'optimal/{args["experiment_type"]}/opt_state_{exp_hash}.eqx', opt_state)
            ...
            with open(f'optimal/{args["experiment_type"]}/results.csv', "a") as f:
                f.write(data)
            
            jnp.savez(f'optimal/{args["experiment_type"]}/metrics_{exp_hash}.npz', cosines=cosines, errors=errors, eigvecs_errors=eigvecs_errors, history=history)