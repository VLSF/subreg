...
def a_rand(X, Y, params, key):
    ind_, w_, alpha, beta, slope = params
...
def get_dataset(N, N_samples, N_eigenvalues, params, key):
    ...
    for keys in Keys:
        ...
        a2_p = a_rand(X, Y_half[:, 1:], params, keys[0])
        a2_m = a_rand(X, Y_half[:, :-1], params, keys[0])
        a1 = a_rand(X, Y, params, keys[0])
        a2 = a_rand(X, Y, params, keys[0])
        ...
        D = A.diagonal() # A_ = splu(A)
        L_ = LinearOperator(A.shape, matvec=lambda x: x - 0.9*A @ x / D, dtype=A.dtype) # A_.solve(x)
...
if __name__ == "__main__":
    ...
    w_ = w.reshape(-1, 1, 1)
    params = [ind_, w_, alpha, beta, slope]
    
    data = get_dataset(N, N_samples, N_eigenvalues, params, key)
    jnp.savez("../datasets/lone_flattop_N100_w0.9_1200samples.npz", **data)
